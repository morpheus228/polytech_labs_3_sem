...
int main()
{
	...
	// Это условие важно для обработки самоприсваивания. 
	// Если операция присваивания вызывается для объекта, который уже является исходным объектом, то нет необходимости выполнять никаких действий. 
	// Этим мы избегаем использования памяти на копирование значения, которое уже находится в этом объекте.
	...
	str = s;
	cout << "\n\nTest MyString operator=" << str.GetString();
	
	// Вызывается ли эта операция в в этом фрагменте? Почему?
	// Потому что в этом случае вызывается констркутор копирования

	MyString ss = s;
	cout << "\n\nTest MyString copy constructor=" << ss.GetString();
	...
	Vector d;
	d = a + b; // Тут кроме сложения вызывается оператор присваивания
	cout << "\n\nTest Vector operator+";
	d.Out();
	
	Vector e = a + b; // Тут кроме сложения вызывается конструктор копирования
